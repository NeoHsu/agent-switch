...
.agent/.sync-manifest.json      generated/link state manifest
```
...
```text
                         ...
                         +---------+----------+
                                   ...
                                   v
+-------------------+      +-------+-------+      +-----------------------------+
| .agent/          | ---> | SyncPlan      | ---> | generated tool-native files |
...
1. explicit `--root`
2. 從 current directory 往上找：`.agent-switch.yaml`、`.agent` 或 `.git`
...
掃描會跳過 `.agent/`、`.claude/`、`.github/`、`.git/` 等工具與管理目錄。
...
Manifest 檔案：`.agent/.sync-manifest.json`
